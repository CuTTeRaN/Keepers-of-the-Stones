...
public class TimeDilationEntity extends AbstractArrow implements ItemSupplier {
	public TimeDilationEntity(PlayMessages.SpawnEntity packet, Level world) {
		super(PowerModEntities.TIME_DILATION.get(), world);
	}
	...
	public ItemStack getItem() {
		return new ItemStack(PowerModItems.TIME_DILATION.get());
	}
	...
	public void onHitEntity(EntityHitResult entityHitResult) {
		...
		TimeDilationUseProcedure.execute(entityHitResult.getEntity());
	}
	...
	public static TimeDilationEntity shoot(Level world, LivingEntity entity, RandomSource random, float power, double damage, int knockback) {
		TimeDilationEntity entityarrow = new TimeDilationEntity(PowerModEntities.TIME_DILATION.get(), entity, world);
	...
	public static TimeDilationEntity shoot(LivingEntity entity, LivingEntity target) {
		TimeDilationEntity entityarrow = new TimeDilationEntity(PowerModEntities.TIME_DILATION.get(), entity, entity.level);
