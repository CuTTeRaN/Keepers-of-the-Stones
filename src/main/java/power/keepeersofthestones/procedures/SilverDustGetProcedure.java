...
public class SilverDustGetProcedure {
	public static void execute(LevelAccessor world, Entity entity) {
		...
		if (!(entity.getCapability(PowerModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new PowerModVariables.PlayerVariables())).selected) {
			if (!PowerModVariables.MapVariables.get(world).silver_dust_stone) {
				if (entity instanceof Player _player) {
					ItemStack _setstack = new ItemStack(PowerModItems.SILVER_DUST_STONE.get());
					_setstack.setCount(1);
					ItemHandlerHelper.giveItemToPlayer(_player, _setstack);
				}
				PowerModVariables.MapVariables.get(world).silver_dust_stone = true;
